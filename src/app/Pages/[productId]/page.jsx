...
export default function Producto() {
    ...
    const [productoConTallas, setProductoConTallas] = useState(false); // Mantener como estado
        ...
        const fetchProducto = async () => {
            ...
            for (const seccion of seccionesArray) {
                ...
                if (productoSnap.exists()) {
                    ...
                    // Verificar si el producto tiene alguna característica de talla (definida, no necesariamente true)
                    const hasTallaProps = 
                        productoData.hasOwnProperty('TallaS') ||
                        productoData.hasOwnProperty('TallaM') ||
                        productoData.hasOwnProperty('TallaL') ||
                        productoData.hasOwnProperty('TallaXL');

                    if (hasTallaProps) {
                        setProductoConTallas(true);
                        setProducto({
                            id: productId,
                            titulo: productoData.Nombre,
                            precio: productoData.Precio,
                            tallaS: productoData.TallaS ?? false, // Usar ?? para asegurar un booleano si es undefined/null
                            tallaM: productoData.TallaM ?? false,
                            tallaL: productoData.TallaL ?? false,
                            tallaXL: productoData.TallaXL ?? false,
                            talla: "Tallas Disponibles", // Este mensaje se puede omitir si el "productoConTallas" lo maneja
                            unidades: productoData.Unidades,
                        });
                    } else {
                        setProductoConTallas(false);
                        setProducto({
                            id: productId,
                            titulo: productoData.Nombre,
                            precio: productoData.Precio,
                            talla: "No aplican tallas al Producto", // Mensaje para productos sin tallas
                            unidades: productoData.Unidades,
                        });
                    }
                    setLoading(false);
                    return; // Importante para salir del bucle una vez que el producto se encuentra
                }
            }
            setLoading(false); // Si el producto no se encuentra en ninguna sección
        };
        fetchProducto();
    }, [productId]);
                ...
                <div className="md:w-1/2 flex flex-col items-start gap-y-5 md:gap-y-12 justify-center">
                    ...
                    <div className="">
                        ...
                        {productoConTallas ? (
                            ...
                            <p className="text-lg mt-2">{producto.talla}</p>
                        )}
                    </div>
                    ...
                    <div className="w-full flex gap-x-4 items-center justify-between">
                        ...
                        <button className="md:hidden block group w-1/2 border-2 border-black py-2 font-semibold rounded-md bg-blue-400 hover:bg-blue-700">
                            <p className="flex gap-x-4 justify-center items-center group-hover:text-white group-hover:scale-150 duration-300">
                                Añadir <br /> a Carrito
